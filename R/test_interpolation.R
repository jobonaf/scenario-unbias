source("R/interpolate-points-to-grid.R")
...
result_tps <- do_interpolation(points_data = data.frame(x = xy[, 1], y = xy[, 2], values = v), 
                               grid_data = r, method = "tps")
plot(result_tps, main = "Thin Plate Spline")

# IDW interpolation
result_idw <- do_interpolation(points_data = data.frame(x = xy[, 1], y = xy[, 2], values = v), 
                               grid_data = r, method = "idw", 
                               params = list(idp = 2, nmax = 5))
plot(result_idw, main = "IDW")

# Ordinary Kriging interpolation
result_ok <- do_interpolation(points_data = data.frame(x = xy[, 1], y = xy[, 2], values = v), 
...
result_ked <- do_interpolation(points_data = data.frame(x = xy[, 1], y = xy[, 2], values = v), 
